...
def offline_predictions(
    arima_p: int, arima_d: int, arima_q: int, data: list[float], no_predictions: int
) -> list[float]:
...
def online_rmse(datasets: list[str], data_type: str) -> tuple[tuple, float]:
    rmses = []
...
def online_predictions(data: list[float], no_predictions: int = 10) -> list[float]:
    data_df = pd.DataFrame(data, columns=["history"])
    train_df, test_df = get_train_test(data_df)
