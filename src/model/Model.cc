...
void Model::addServer(double bootDelay)
{
    ...
    std::cout << simTime().dbl() << ": "
              << "addServer(" << bootDelay << "): serverCount=" << getServers() << " active=" << activeServers << " expected=" << events.size() << std::endl;
#endif
}
...
#if LOCDEBUG
    std::cout << simTime().dbl() << ": "
              << "serverBecameActive(): serverCount=" << getServers() << " active=" << activeServers << " expected=" << events.size() << std::endl;
    if (events.size() > 0)
    {
...
#if LOCDEBUG
    std::cout << simTime().dbl() << ": "
              << "removeServer(): serverCount=" << getServers() << " active=" << activeServers << " expected=" << events.size() << std::endl;
#endif
}
...
void Model::updateServiceTimeHistory(double serviceTimeMean)
{
